...
function draw() {
    ...
    // ===== D2 KÉO DÀI 2 PHÍA =====
    // giao với cạnh trái (x = ox)
    const tLeft = (ox - mx) / dx;
    const yLeft = my + dy * tLeft;

    // giao với cạnh phải (x = rightX)
    const tRight = (rightX - mx) / dx;
    const yRight = my + dy * tRight;
    ...
    // ===== KÉO DÀI ĐƯỜNG ĐỎ ĐẾN D1 =====

    // điểm giao thật với D1
    const yOnD1 = yIntersect;

    // vẽ line full từ circle dưới → D1
    svg.appendChild(create("line", {
        x1: x,
        y1: cy2,          // tâm circle trên
        x2: x,
        y2: yIntersect,   // giao với D1
        stroke: "red",
        "stroke-width": 1.5
    }));

        // ===== TEXT 50 / 100 (FIXED LEFT ALIGN) =====

        // khoảng cách từ circle ra text
        const textGap = 10;

        // vị trí X luôn nằm bên trái circle
        const textX = x - r - textGap;

        // ===== TEXT 100 (circle trên) =====
        svg.appendChild(create("text", {
            x: textX,
            y: cy2,
            "text-anchor": "end",          // canh phải → text dính sát vào điểm X
            "dominant-baseline": "middle", // căn giữa theo Y
            "font-size": "11",
            fill: "black"
        }, "100"));
